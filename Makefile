...
MLFILES:=g_local_ascii_syntax.ml\
  g_local_string_syntax.ml
...
all: $(VOFILES) $(CMOFILES) $(CMXSFILES) Ascii_syntax.vo\
  String_syntax.vo
spec: $(VIFILES)
...
Ascii_syntax.vo: 
	$(COQC) $(COQLIBS) -q -byte Ascii_syntax.v

String_syntax.vo: 
	$(COQC) $(COQLIBS) -q -byte String_syntax.v
...
install:
	...
	(for i in $(VOFILES); do \
	 install -d `dirname $(COQLIB)/user-contrib/String/$$i`; \
	 install $$i $(COQLIB)/user-contrib/String/$$i; \
	 done)
...
clean:
	...
	- rm -rf html
	- rm -f Ascii_syntax.vo
	- rm -f String_syntax.vo
