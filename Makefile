...
ML4FILES:=g_local_ascii_syntax.ml4\
  g_local_string_syntax.ml4
...
all: $(VOFILES) $(CMOFILES) $(if $(HASNATDYNLINK_OR_EMPTY),$(CMXSFILES)) g_local_string_syntax.cmo\
  g_local_string_syntax.cmx
...
g_local_string_syntax.cmo: g_local_ascii_syntax.cmo

g_local_string_syntax.cmx: g_local_ascii_syntax.cmo
...
uninstall_me.sh: Makefile
	...
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/String && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "String" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
...
clean::
	...
	- rm -rf g_local_string_syntax.cmo
	- rm -rf g_local_string_syntax.cmx
